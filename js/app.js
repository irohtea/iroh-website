(() => {
    ...
    function schemeChange() {
        window.addEventListener("load", (function() {
            ...
            let userTheme;
            if (window.matchMedia) if (window.matchMedia("(prefers-color-scheme: dark)").matches) userTheme = "dark"; else userTheme = "light";
            window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", (e => {
                if (!saveUserTheme) changeTheme(); else null;
            }));
            ...
            function changeTheme(saveTheme = false) {
                let currentTheme;
                let newTheme;
                if (htmlBlock.classList.contains("light")) currentTheme = "light"; else currentTheme = "dark";
                if ("light" === currentTheme) newTheme = "dark"; else if ("dark" === currentTheme) newTheme = "light";
                htmlBlock.classList.remove(currentTheme);
                htmlBlock.classList.add(newTheme);
                if (saveTheme) localStorage.setItem("user-theme", newTheme); else null;
            }
        }));
    }
    function scrollUpButton() {
        const scrollBtn = document.querySelector(".scroll-to-top");
        const offset = scrollBtn.dataset.height;
        const getTop = () => window.scrollY || document.documentElement.scrollTop;
        window.addEventListener("scroll", (() => {
            if (getTop() > offset) scrollBtn.classList.add("active"); else scrollBtn.classList.remove("active");
        }));
    ...
    let formValidate = {
        ...
        telTest(formRequiredItem) {
            return !/^((\+?3)?8)?((0\(\d{2}\)?)|(\(0\d{2}\))|(0\d{2}))\d{7}$/.test(formRequiredItem.value);
        }
    };
    ...
    let itemWorks = document.querySelectorAll(".item-works");
    filterItem.forEach(((filter, i) => filter.addEventListener("click", (() => {
        filterItem.forEach((filter => filter.classList.remove("active")));
        filter.classList.add("active");
        let datafilter = filter.getAttribute("data-filter");
        itemWorks.forEach((item => {
            item.classList.add("hide");
            if (item.getAttribute("data-category") === datafilter || "all" === datafilter) {
        ...
        entries.forEach((entry => {
            if (entry.isIntersecting) {
                ...
                document.querySelectorAll(".contents__link").forEach((link => {
                    if (link.getAttribute("href").replace("#", "") === entry.target.id) link.classList.add("_active"); else link.classList.remove("_active");
                }));
            }
        }));
    }), {
